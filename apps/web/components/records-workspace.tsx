...
export function RecordsWorkspace({
  ...
  const { records, loading, add, update, remove } = useLifeRecords();
  const today = new Date().toISOString().slice(0, 10);
  const allowedKinds = subsection
    ? subsectionKinds[module]?.[subsection]
    : undefined;
  const defaultKind = allowedKinds?.[0] ?? kindOptions[module][0];
  const workspaceLabel = subsection
    ? subsection.replaceAll("-", " ")
    : moduleLabels[module];
  const [query, setQuery] = useState("");
  const [statusFilter, setStatusFilter] = useState<
    "all" | LifeRecord["status"]
  >("all");
  const items = useMemo(
    () =>
      recordsInSection(records, module, subsection, today)
        .filter(
    ...
    [records, module, subsection, today, statusFilter, query],
  );
  const [open, setOpen] = useState(false);
  const [editing, setEditing] = useState<string>();
  const [draft, setDraft] = useState<Draft>(() => blank(defaultKind));
  ...
  // `close` does not bubble, so React's onClose prop never fires. Escape and
  // backdrop dismissal both reach state through this listener.
  useEffect(() => {
    const element = dialog.current;
    if (!element) return;
  ...
  const save = (event: React.FormEvent) => {
    ...
    const values = {
      ...
      progress: draft.progress
        ? Math.max(0, Math.min(100, Number(draft.progress)))
        : undefined,
      essential: draft.essential,
    };
    if (editing) update(editing, values);
    else add(values);
    setOpen(false);
  };
    ...
    <section className="records-section">
      <div className="records-heading">
        <div>
          ...
          <h2>
            {workspaceLabel[0]?.toUpperCase()}
            {workspaceLabel.slice(1)} workspace
          </h2>
          <p>
            {counts.total} shown · {counts.done} completed · {counts.upcoming}{" "}
            upcoming
          </p>
        </div>
        <button className="button primary" onClick={() => begin()}>
          <Plus size={16} /> Add {defaultKind.toLowerCase()}
        </button>
      </div>
      ...
      <div className="records-list">
        {items.length ? (
          items.map((record) => (
            <article
              className={`record-row ${record.status === "done" ? "complete" : ""}`}
              key={record.id}
            >
              <button
                ...
                onClick={() =>
                  update(record.id, {
                    ...
                    progress: record.status === "done" ? record.progress : 100,
                  })
                }
                aria-label={`${record.status === "done" ? "Reopen" : "Complete"} ${record.title}`}
              >
                {record.status === "done" ? (
                  <Check size={15} />
                ) : (
              ...
              <div className="record-copy">
                <div>
                  <span className={`record-kind ${module}`}>{record.kind}</span>
                  {record.essential && (
                    <span className="record-kind essential">
                      Today essential
                    </span>
                  )}
                </div>
                <h3>{record.title}</h3>
                <p>{record.detail || "No extra details yet."}</p>
          ...
          <div className="records-empty" aria-live="polite">
          ...
          <div className="records-empty">
            ...
            <p>
              {query || statusFilter !== "all"
                ...
                : "Add only information that helps future you remember, decide or find something."}
            </p>
            {query || statusFilter !== "all" ? (
              <button
      ...
      >
        {open && (
          <form className="modal-card record-form" onSubmit={save}>
            <div className="modal-heading">
              <div>
                <span className="eyebrow">{moduleLabels[module]}</span>
                <h2 id="record-form-title">
                  {editing ? "Edit item" : "Add something useful"}
                </h2>
              </div>
              <button
                className="icon-button"
            ...
            <div className="form-grid">
              <label className="input-row">
                ...
                >
                  {(allowedKinds ?? kindOptions[module]).map((kind) => (
              ...
              <label className="input-row">
                ...
                <select
                  ...
                  onChange={(e) =>
                    setDraft({
                      ...draft,
                      status: e.target.value as LifeRecord["status"],
                    })
                  }
                >
                  <option value="open">Open</option>
            ...
            <label className="input-row">
              ...
              <textarea
                ...
                placeholder="What will you need to know later?"
              />
            </label>
            <div className="form-grid">
              <label className="input-row">
            ...
            <label className="toggle-row compact">
              <span>
                <strong>Show as a Today essential</strong>
                <small>Use this only for the few things that matter now.</small>
              </span>
              <input
                type="checkbox"
                checked={draft.essential}
                onChange={(e) =>
                  setDraft({ ...draft, essential: e.target.checked })
                }
              />
            </label>
